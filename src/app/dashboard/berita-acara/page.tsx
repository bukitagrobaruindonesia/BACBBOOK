      ...
      spPagesHtml += `
        <div class="page-sp">
          ...
          <div class="table-section">
            ...
            <table class="data-table">
              <tbody>
                <tr><td style="padding:6px 8px;font-size:10px;border:1px solid #000;font-weight:600;width:120px;">NO. POLISI :</td><td style="padding:6px 8px;font-size:10px;border:1px solid #000;">${surat.nomorPolisi || "-"}</td></tr>
                <tr><td style="padding:6px 8px;font-size:10px;border:1px solid #000;font-weight:600;">DRIVER UNIT :</td><td style="padding:6px 8px;font-size:10px;border:1px solid #000;">${surat.driverUnit || "-"}</td></tr>
                <tr><td style="padding:6px 8px;font-size:10px;border:1px solid #000;font-weight:600;">NOMOR SIM :</td><td style="padding:6px 8px;font-size:10px;border:1px solid #000;">${surat.nomorSIM || "-"}</td></tr>
              </tbody>
            </table>
          </div>
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .page-pi { width: 182mm; margin: 0 auto; position: relative; min-height: 257mm; }
    .page-sp { width: 176mm; margin: 0 auto; position: relative; min-height: 257mm; display: flex; flex-direction: column; }
    ...
    .sp-title-bar { text-align: center; background: #15803d; color: white; padding: 8px 0; margin: 8px 0 12px 0; font-weight: bold; font-size: 14px; letter-spacing: 2px; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    .sp-info-section { margin-bottom: 12px; }
    .sp-info-row { display: flex; justify-content: space-between; margin-bottom: 4px; font-size: 10px; }
    .sp-info-label { font-weight: 600; }
    .sp-recipient-box { border: 1px solid #000; padding: 8px 10px; margin-bottom: 10px; }
    .sp-recipient-title { font-size: 9px; color: #333; margin-bottom: 2px; }
    .sp-recipient-name { font-size: 11px; font-weight: 700; }
    .sp-recipient-address { font-size: 9px; color: #333; line-height: 1.5; margin-top: 2px; }
    .sp-salutation { font-size: 10px; margin-bottom: 8px; }
    .sp-salutation p { margin-bottom: 2px; }
    .sp-table-section { margin-bottom: 10px; }
    .sp-table-title { text-align: center; background: #dcfce7; border: 1px solid #000; border-bottom: none; padding: 4px 0; font-size: 10px; font-weight: 700; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    .sp-notes-section { margin-top: 10px; font-size: 9px; }
    .sp-notes-section p { margin-bottom: 2px; }
    .sp-signature-row { display: flex; justify-content: space-between; margin-top: auto; padding-top: 20px; align-items: flex-end; }
    .sp-signature-box { width: 45%; text-align: center; display: flex; flex-direction: column; justify-content: flex-end; align-items: center; }
    .sp-signature-title { font-size: 9px; margin-bottom: 4px; min-height: 28px; line-height: 1.4; }
    .sp-signature-img { max-height: 60px; width: auto; object-fit: contain; margin: 0 auto 4px auto; display: block; }
    .sp-signature-name { font-size: 10px; font-weight: 700; margin-top: 0; border-top: 1px solid #000; padding-top: 3px; display: block; width: 90%; margin-left: auto; margin-right: auto; }
  ...
  <div class="page">
    ...
    <div class="content">
      ...
      <div class="party-section">
        ...
        <table class="party-table">
          <tr><td class="party-label">Nama</td><td>: ${ba.pihakKedua?.nama || pi.namaCustomer || ""}</td></tr>
          <tr><td class="party-label">Alamat</td><td>: ${(ba.pihakKedua?.alamat || pi.alamatCustomer || "").replace(/\n/g, " ")}</td></tr>
        </table>
      </div>
      ...
      <div class="signature-row">
        <div class="signature-box">
          <p class="signature-title">${pi.namaCustomer}<br>(Pihak Kedua)</p>
          <div style="height:50px;"></div>
          <p class="signature-name">${pi.namaCustomer}</p>
        </div>
        <div class="signature-box">
          ...
          <p class="signature-name">${ttd?.nama || "_________________"}</p>
          <p class="signature-role">${ttd?.jabatan || "PT Bukit Agrochemical Baru"}</p>
        </div>
      </div>
    </div>
  </div>
    ...
    <div style="position:relative;z-index:1;">
      ...
      <div class="info-section">
        ...
        <div class="info-row">
          <div class="customer-box">
            ...
            <p class="customer-address">${(pi.alamatCustomer || "").replace(/\n/g, "<br>")}</p>
          </div>
          <div class="invoice-meta">
            <div class="meta-row"><span class="meta-label">Tanggal</span><span class="meta-colon">:</span><span class="meta-value">${pi.tanggal || ""}</span></div>
            <div class="meta-row"><span class="meta-label">No Invoice</span><span class="meta-colon">:</span><span class="meta-value">${pi.nomorPI || ""}</span></div>
            <div class="meta-row"><span class="meta-label">Metode Pembayaran</span><span class="meta-colon">:</span><span class="meta-value">${pi.metodePembayaran || ""}</span></div>
            ${pi.npwp ? `<div class="meta-row"><span class="meta-label">NPWP</span><span class="meta-colon">:</span><span class="meta-value">${pi.npwp}</span></div>` : ""}
          </div>
        </div>
      </div>
      <table class="data-table">
      ...
      <div class="summary-row">
        <div class="terbilang-area">
          ...
          <div class="terbilang-text">${pi.terbilang || "-"}</div>
        </div>
        <div class="calc-area">
          <div class="calc-line"><span class="calc-name">Subtotal</span><span class="calc-amount">${formatRupiah(pi.subtotal)}</span></div>
          ${(pi.uangMuka || 0) > 0 ? `<div class="calc-line"><span class="calc-name">Uang Muka</span><span class="calc-amount">${formatRupiah(pi.uangMuka)}</span></div>` : ""}
          ${pi.includePPN ? `<div class="calc-line"><span class="calc-name">PPN 11%</span><span class="calc-amount">${formatRupiah(pi.ppnNominal)}</span></div>` : ""}
          ${(pi.ongkosKirim || 0) > 0 ? `<div class="calc-line"><span class="calc-name">Ongkos Kirim</span><span class="calc-amount">${formatRupiah(pi.ongkosKirim)}</span></div>` : ""}
          <div class="calc-line"><span class="calc-name-bold">Jumlah Tertagih</span><span class="calc-amount-bold">${formatRupiah(pi.jumlahTertagih)}</span></div>
          <div class="due-date"><span class="due-label">Tanggal Jatuh Tempo : </span><span class="due-value">${pi.tanggalJatuhTempo || ""}</span></div>
      ...
      <div class="footer-row">
        ...
        <div class="footer-ttd-area">
          ...
          ${pi.ttdImage ? `<img src="${pi.ttdImage}" class="ttd-img" alt="TTD" />` : `<div style="height:40px;"></div>`}
          <p class="ttd-name">${pi.ttdNama || ""}</p>
          <p class="ttd-role">${pi.ttdJabatan ? `(${pi.ttdJabatan})` : ""}</p>
        </div>
      </div>
    </div>
  </div>
