  ...
  const handleResetInvoiceBase = async (row: ProformaInvoice) => {
    ...
    try {
      ...
      for (const surat of allSurat) {
        ...
        await updateDoc(suratRef, { updatedAt: serverTimestamp() });
      }
  ...
  const handleTerbitkanInvoice = async () => {
    ...
    try {
      ...
      const arsipData = {
        ...
        riwayatPengangkutan: getSuratMuatForPI(pi.nomorPI).map((s) => ({
            ...
            <div className="space-y-1">
              {produkStatus.map((p, i) => (
                <div key={i} className="text-xs text-gray-600">
                  ...
                  {p.isGI ? <span className="text-green-700 ml-1 font-bold">[GI]</span> : <span className="text-blue-700 ml-1 font-bold">[DO]</span>}
                </div>
              ))}
            </div>
                ...
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                    {getSuratMuatForPI(selectedItem.nomorPI).map((surat: SuratMuatInfo, idx: number) => {
                      const suratIsGI = !surat.jenisSurat || surat.jenisSurat === "gudangInduk";
                      const suratIsMandiri = surat.jenisSurat === "do" && surat.subJenisDO === "mandiri";
                      const suratIsDikuasakan = surat.jenisSurat === "do" && surat.subJenisDO === "dikuasakan";
