...
export default function PenggantiBarangRusakPage() {
  ...
  const handleSubmitPenggantian = async (e: React.FormEvent) => {
    ...
    try {
      ...
      assignedNomorBA = await getUniqueBANumber("pengganti");
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-amber-50">
                  ...
                  <th className="px-3 py-2 text-left text-xs font-semibold text-amber-800 uppercase border">Nomor BA</th>
              ...
              <tbody>
                {tidakDigantiHistory.map((r, idx) => (
                  <tr key={`${r.transaksiId}_${r.rusakIndex}`} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2 text-sm text-amber-700 text-right font-mono border font-bold">{r.jumlahTidakDiganti.toLocaleString("id-ID")} {r.rusakUnit}</td>
                    <td className="px-3 py-2 text-sm text-red-700 text-right font-mono border">{r.sisaRusak.toLocaleString("id-ID")} {r.rusakUnit}</td>
