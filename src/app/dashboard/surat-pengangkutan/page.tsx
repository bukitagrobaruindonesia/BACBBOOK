...
export default function SuratPengangkutanPage() {
  ...
  const calculatePILoadInfo = async () => {
    ...
    const filteredPIList = piList.filter((pi) => {
      ...
      return (
        pi.nomorPI.toLowerCase().includes(searchLower) ||
        pi.namaCustomer.toLowerCase().includes(searchLower)
      );
    });
      ...
      setItems((prev) =>
        prev.map((item) => {
          ...
          if (field === "pengambilanZAK") {
            ...
            const maxKG = item.maxZAK * item.bobotPerUnit;
            if (kg > maxKG) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (jenisSurat === "gudangInduk" && selectedPI) {
          ...
          const remainingKG = piLoadInfo ? piLoadInfo.remainingKG : 0;
          const newRemaining = Math.max(0, remainingKG - totalPengambilanKG);

          const piRef = doc(db, "proformaInvoice", selectedPI.id);
          await updateDoc(piRef, {
            sisaPengambilanKG: newRemaining,
            statusPengangkutan: newRemaining <= 0 ? "complete" : "partial",
            updatedAt: serverTimestamp(),
          });
                        ...
                        filteredPIList.map((pi) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <p className="text-xs text-green-600 mt-1">Total: {totalOrdered.toLocaleString()} KG</p>
                            </button>
                          );
                        })
                      )}
